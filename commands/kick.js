...
module.exports = {
  ...
  execute: async (interaction, utils) => {
    ...
    if (member.roles.highest.position >= interaction.member.roles.highest.position) {
      return interaction.reply({ content: 'You cannot kick this user because they have a higher or equal role.', ephemeral: true });
    }

    // Check role hierarchy: Bot cannot kick members with higher roles than itself
    if (member.roles.highest.position >= interaction.guild.members.me.roles.highest.position) {
      return interaction.reply({ content: 'I cannot kick this user because their role is higher than mine.', ephemeral: true });
    }
