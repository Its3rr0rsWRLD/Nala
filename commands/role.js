...
module.exports = {
  ...
  execute: async (interaction) => {
    ...
    if (!selfAssignableRoles.includes(role.id)) {
      return interaction.reply({ content: 'You cannot assign or remove this role.', ephemeral: true });
    }
    ...
    if (subcommand === 'add') {
      ...
      if (!member.roles.cache.has(role.id)) {
        return interaction.reply({ content: 'You do not have this role.', ephemeral: true });
      }
