...
module.exports = {
  ...
  execute: async (interaction, utils) => {
    ...
    if (member.roles.highest.position >= interaction.member.roles.highest.position) {
      return interaction.reply({ content: 'You cannot ban this user because they have a higher or equal role.', ephemeral: true });
    }
    ...
    if (settings.alertBanToUser) {
      const banEmbed = {
        ...
        fields: [
          ...
          { name: 'Duration', value: durationStr || 'Permanent', inline: false },
        ],
        timestamp: new Date(),
      };
    ...
    try {
      ...
      await interaction.reply({
        content: `Successfully banned ${user.tag}${durationStr ? ` for ${durationStr}` : ''}.`,
        ephemeral: true,
      });
    } catch (error) {
